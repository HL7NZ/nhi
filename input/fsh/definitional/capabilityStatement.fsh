...
//comma delimited fields in documentation string extracted by makeCapabilityStatement.js:   <verb,url,doc, scope>
* rest.resource.operation.documentation = "POST, [base]/Patient/$match, <b>Match</b>: with In parameter <code>onlyCertainMatches=0</code> (False), the operation will search the NHI by demographics using fuzzy logic. Returns a bundle of results with match scores. <br></br> <b>Validate</b>: with In parameter <code>onlyCertainMatches=1</code> (True) the operation will validate known patient demographic details against an NHI number. Returns a bundle of results with one patient resource that only includes the live nhi-id.,  system/Patient.u system/Patient.v"
