...
* rest.resource.operation[8].documentation = "An operation to replace a name on a patient's record.  **HTTP Request** 'POST'.  **Scope required:**  system/Patient.u, system/Patient.a" 
...
// * rest.resource.operation[11].documentation = "An operation to update a patient's death details.  **HTTP Request** 'POST'.  **Scope required:**  system/Patient.u, system/Patient.a" 
